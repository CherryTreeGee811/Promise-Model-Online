...
export const updateMomentOwner = (owner: string, project: string, momentSeq: string | number, userId: string | number | undefined, flowId?: number) => apiPatch(withFlowId(`/api/projects/${encodeURIComponent(owner)}/${encodeURIComponent(project)}/moments/${momentSeq}/owner`, flowId), { userId });

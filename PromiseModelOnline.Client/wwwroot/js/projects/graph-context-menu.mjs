...
// Stripped the /api prefix because appApi handles the base URL automatically
...
function getCreateActionMeta(nodeData) {
    ...
    switch (normalizedType) {
        case 'root':
            return { entityLabel: 'Promise', endpoint: '/promises/create', parentField: 'projectId' };
        case 'promise':
            return { entityLabel: 'Epic', endpoint: '/epics/create', parentField: 'productPromiseId' };
        case 'epic':
            return { entityLabel: 'Journey', endpoint: '/journeys/create', parentField: 'epicId' };
        case 'journey':
            return { entityLabel: 'Flow', endpoint: '/flows/create', parentField: 'journeyId' };
        case 'flow':
            return { entityLabel: 'Moment', endpoint: '/moments/create', parentField: 'flowId' };
        default:
            return null;
    }
}
...
function getCreateFormDefaults(nodeData) {
    ...
    switch (normalizedType) {
        case 'root':
            return { statement: 'New Promise', description: '', displayOrder: nextDisplayOrder };
        case 'promise':
            return { statement: 'New Epic', description: '', displayOrder: nextDisplayOrder };
        case 'epic':
            return { statement: 'New Journey', description: '', displayOrder: nextDisplayOrder };
        case 'journey':
            return { statement: 'New Flow', description: '', displayOrder: nextDisplayOrder };
        case 'flow':
            return { statement: 'New Moment', description: '', displayOrder: nextDisplayOrder };
        default:
            return null;
    }
}
...
function buildMomentFormElement(nodeData, projectId, getAvailableStrides, onGraphMutated, closeMenus) {
    ...
    form.addEventListener('submit', async event => {
        ...
        try {
            await appApi.post(createMeta.endpoint, payload);
            closeMenus();

...
function e(e){let t={...e},n=new Set;return{get(){return t},set(e){t={...t,...e},n.forEach(e=>{typeof e==`function`&&e()})},subscribe(e){return n.add(e),()=>{n.delete(e)}}}}var t=`pmo.auth`;function n(){try{let e=sessionStorage.getItem(t);if(e){let t=JSON.parse(e);return{isAuthenticated:!!t.isAuthenticated,username:t.username||null,userId:t.userId??null}}}catch{}return{isAuthenticated:!1,username:null,userId:null}}function r(e){try{sessionStorage.setItem(t,JSON.stringify(e))}catch{}}var i=e(n()),a=i.set.bind(i);i.set=e=>{a(e),r(i.get())};function o(){return i.get().isAuthenticated}function s(){return i.get().username}function c(){return i.get().userId}function l(){let e=o()?`
      ...
      </a>`,t=document.getElementById(`home-cta-area`),n=document.getElementById(`home-cta-area-bottom`);t&&(t.innerHTML=e),n&&(n.innerHTML=e)}async function u(e){let t=await m(e);if(t.status===204)return null;if(!t.ok)throw Error(`HTTP ${t.status}`);return t.json()}async function d(e){return await u(e)??[]}async function f(e,t){let n=await m(e,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(t)});if(n.status===204)return null;if(!n.ok)throw Error(`HTTP ${n.status}`);return n.json()}async function p(e,t){let n=await m(e,{method:`PATCH`,headers:{"Content-Type":`application/json`},body:JSON.stringify(t)});if(n.status===204)return null;if(!n.ok)throw Error(`HTTP ${n.status}`);return n.json()}async function m(e,t={}){let n=await fetch(e,{...t,credentials:`include`,headers:{Accept:`application/json`,...t.headers||{}}});if(n.status===401)throw i.set({isAuthenticated:!1,username:null,userId:null}),window.location.pathname.startsWith(`/login`)||(window.location.href=`/login`),Error(`Unauthorized`);return n}async function h(){try{let e=await fetch(`/api/users/me`,{method:`GET`,credentials:`include`});if(e.ok){let t=await e.json();return i.set({isAuthenticated:!0,username:t.name,userId:t.userId}),!0}}catch{}return i.set({isAuthenticated:!1,username:null,userId:null}),!1}async function g(){try{return await u(`/api/notifications`)??[]}catch{return[]}}var _=g,v=g,ee=e=>p(`/api/notifications/${e}`,{isRead:!0}),y=()=>p(`/api/notifications`,{isRead:!0,applyToAll:!0}),b=null,x=null;async function te(e){if(!b){x=typeof e==`function`?e:null,b=new signalR.HubConnectionBuilder().withUrl(`/hubs/notifications`).withAutomaticReconnect([0,2e3,5e3,1e4,3e4]).configureLogging(signalR.LogLevel.Warning).build(),b.on(`ReceiveNotification`,e=>{x&&x(e)}),b.onreconnected(async()=>{x&&x(null)});try{await b.start()}catch(e){console.warn(`SignalR connection failed, notifications will not be real-time:`,e),b=null}}}var S=`pmo:notifications:unread-updated`,C=!1;function w(e){let t=document.getElementById(`notification-badge`);if(!t)return;let n=Number.isFinite(e)?e:0;n>0?(t.textContent=String(n),t.classList.remove(`d-none`)):t.classList.add(`d-none`)}async function T(){try{let e=await _();w(Array.isArray(e)?e.length:0),window.dispatchEvent(new CustomEvent(S,{detail:{notifications:Array.isArray(e)?e:[]}}))}catch{w(0)}}async function E(){await T()}function ne(){T(),!C&&(C=!0,te(()=>{T()}))}function re(){return S}function ie(){let e=window.location.pathname;document.querySelectorAll(`#main-menu a[data-nav]`).forEach(t=>{let n=t.getAttribute(`href`);if(!n||n===`#`)return;let r=n===e||n!==`/`&&e.startsWith(n);t.removeAttribute(`aria-current`),r&&t.setAttribute(`aria-current`,`page`)})}function ae(e,t,n){let r=e.target.closest(`a[data-nav]`);if(!r)return;let i=r.getAttribute(`href`);!i||i===`#`||(e.preventDefault(),q(i,t,n))}function oe(e,t){let n=o()?`authenticated.html`:`anonymous.html`;return fetch(`/templates/navigation/${n}`).then(e=>{if(!e.ok)throw Error(`Network response was not ok`);return e.text()}).then(t=>{e.innerHTML=t,ie(),o()&&ne()}).catch(t=>{throw e.innerHTML=`<h1>Error loading template</h1><p>${t.message}</p>`,t})}function se(e,t){let n=document.getElementById(`main-menu`);!n||n.dataset.navBound||(n.dataset.navBound=`1`,n.addEventListener(`click`,n=>ae(n,e,t)))}function D(){return o()?{allowed:!0}:{allowed:!1,redirect:`/login`}}var ce=(e,t,n)=>u(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}`),le=(e,t,n)=>f(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/create`,n),ue=()=>u(`/api/moments/assigned-to-me`),de=(e,t,n,r)=>p(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}/stride-assignment`,{strideId:r}),fe=(e,t,n,r)=>p(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}/status`,{newStatus:r}),pe=(e,t,n,r)=>p(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}/estimate`,{estimate:r}),O=(e,t,n,r)=>p(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}/type`,{newType:r}),k=(e,t,n,r)=>p(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}/owner`,{userId:r}),A=(e,t,n,r)=>p(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}/description`,{description:r}),j=(e,t,n,r)=>f(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}/tasks`,r),M=(e,t,n,r,i)=>p(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}/tasks/${r}/completion`,{isCompleted:i});function N(e){return String(e).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e])}function P(e){return`
        <div class="d-flex w-100 justify-content-center align-items-center py-5" aria-live="polite">
            <div class="spinner-border text-primary" role="status" aria-label="${N(e)}">
                <span class="visually-hidden">${N(e)}</span>
            </div>
        </div>
    `}function F({icon:e,title:t,description:n,button:r,colspan:i}){return`
        <tr class="inline-table-empty-row">
            <td colspan="${i}" class="text-center py-5">
                <div class="d-flex flex-column align-items-center gap-3">
                    ${e?`<div class="empty-table-icon"><i class="bi ${N(e)}"></i></div>`:``}
                    ${t?`<h5 class="fw-semibold text-secondary mb-1">${N(t)}</h5>`:``}
                    ${n?`<p class="text-muted mb-2">${N(n)}</p>`:``}
                    ${r?`<button${r.id?` id="${N(r.id)}"`:``} class="${N(r.class||`btn btn-outline-primary rounded-pill mt-2`)}" type="button">${r.icon?`<i class="bi ${N(r.icon)} me-1"></i> `:``}${N(r.text)}</button>`:``}
                </div>
            </td>
        </tr>
    `}function I({icon:e,title:t,description:n,button:r}){return`
    ...
    `}function L(e,t){let n=document.getElementById(`my-tasks-content`),r=document.getElementById(`error-text`);ue().then(r=>{if(!r||r.length===0){n.innerHTML=I({icon:`bi-list-task`,title:`You have no assigned tasks.`,description:`When a moment is assigned to you, it will appear here.`});return}n.innerHTML=`
            ...
            `,n.addEventListener(`change`,async e=>{let t=e.target;if(t.matches(`.moment-type-select`)){let e=t.closest(`tr`),n=e?.dataset.owner,r=e?.dataset.project;if(!n||!r){console.error(`Cannot determine project for moment type update`);return}let i=parseInt(t.dataset.momentId,10),a=t.value,o=t.dataset.currentType||a;try{await O(n,r,i,a),t.dataset.currentType=a}catch(e){t.value=o,console.error(`Failed to update moment type:`,e)}}}),n.querySelectorAll(`a[moment-seq]`).forEach(n=>{n.addEventListener(`click`,r=>{r.ctrlKey||r.metaKey||r.button===1||(r.preventDefault(),q(`/${n.getAttribute(`data-owner`)}/${n.getAttribute(`data-project`)}/moments/${n.getAttribute(`moment-seq`)}`,e,t))})})}).catch(e=>{r.textContent=`Failed to load your tasks.`,console.error(e)})}var R=!1;function z(e){let t=document.getElementById(`notification-badge`);if(!t)return;let n=Number.isFinite(e)?e:0;n>0?(t.textContent=String(n),t.style.display=`inline`):t.style.display=`none`}function B(){let e=document.getElementById(`notification-badge`);if(!e||e.style.display===`none`)return;let t=parseInt(e.textContent||`0`,10);if(!Number.isFinite(t)||t<=0){z(0);return}z(t-1)}function me(e){if(!e)return;let t=e.classList.contains(`unread`);e.classList.remove(`unread`);let n=e.querySelector(`td[data-actions="1"]`);n&&(n.textContent=`✓ Read`),t&&B()}function V(e,t){if(e){if(!t||t.length===0){e.innerHTML=I({icon:`bi-bell`,title:`No notifications yet.`,description:`You'll see notifications here when there is activity related to you.`});return}e.innerHTML=`
        <div class="mb-3">
    ...
    `,document.getElementById(`mark-all-read`)?.addEventListener(`click`,async()=>{try{await y();let t=window.scrollY;e.querySelectorAll(`tbody tr`).forEach(e=>{e.classList.remove(`unread`);let t=e.querySelector(`td[data-actions="1"]`);t&&(t.textContent=`✓ Read`)}),z(0),window.scrollTo(0,t)}catch(e){alert(`Failed to mark all as read`),console.error(e)}}),e.querySelectorAll(`.mark-read-btn`).forEach(t=>{t.addEventListener(`click`,async()=>{let n=parseInt(t.dataset.id,10);try{await ee(n);let t=window.scrollY;me(e.querySelector(`tr[data-notification-id="${n}"]`)),window.scrollTo(0,t)}catch(e){alert(`Failed to mark notification as read`),console.error(e)}})})}}async function he(){let e=document.getElementById(`notifications-list`),t=document.getElementById(`error-text`);if(!(!e||!t)){t.textContent=``;try{V(e,await v()),E()}catch{t.textContent=`Failed to load notifications.`}}}function ge(e){if(!R){R=!0;let e=re();window.addEventListener(e,e=>{let t=document.getElementById(`notifications-list`);if(!t)return;let n=e?.detail?.notifications;V(t,Array.isArray(n)?n:[])})}he()}function _e(e,t,n){e===`/notifications`?X(`notifications/list.html`,n).then(()=>ge(n)).catch(Q(n,`notifications`)):Z(n)}var ve=()=>u(`/api/permissions/pending`),ye=e=>p(`/api/permissions/${e}`,{status:`Active`});function be(e){let t=document.getElementById(`invitations-list`),n=document.getElementById(`error-text`);async function r(){try{let e=await ve();if(!e||e.length===0){t.innerHTML=I({icon:`bi-envelope`,title:`No pending invitations.`,description:`When someone invites you to a project, it will appear here.`});return}t.innerHTML=`
                <table class="table table-sm table-striped table-hover align-middle">
                    <thead>
                        <tr>
                            <th>Project</th>
            ...
            `,document.querySelectorAll(`.accept-btn`).forEach(e=>{e.addEventListener(`click`,async()=>{let n=parseInt(e.dataset.permissionId,10);try{await ye(n);let r=window.scrollY;e.closest(`tr`)?.remove(),t.querySelectorAll(`tbody tr`).length===0&&(t.innerHTML=I({icon:`bi-envelope`,title:`No pending invitations.`,description:`When someone invites you to a project, it will appear here.`})),window.scrollTo(0,r)}catch(e){alert(`Failed to accept invitation`),console.error(e)}})})}catch{n.textContent=`Failed to load invitations.`}}r()}function xe(e,t){e===`/invitations`?X(`invitations/list.html`,t).then(()=>be(t)).catch(Q(t,`invitations`)):Z(t)}function H(e,t,n){return`
    <section id="${e}" class="kb-section">
      <h1>${t}</h1>
      ${n}
    </section>
  `}function Se(){let e=document.getElementById(`kb-content`);e.innerHTML=[H(`overview`,`The Promise Stack Overview`,`

...
function e(e){let t={...e},n=new Set;return{get(){return t},set(e){t={...t,...e},n.forEach(e=>{typeof e==`function`&&e()})},subscribe(e){return n.add(e),()=>{n.delete(e)}}}}var t=`pmo.auth`;function n(){try{let e=sessionStorage.getItem(t);if(e){let t=JSON.parse(e);return{isAuthenticated:!!t.isAuthenticated,username:t.username||null,userId:t.userId??null}}}catch{}return{isAuthenticated:!1,username:null,userId:null}}function r(e){try{sessionStorage.setItem(t,JSON.stringify(e))}catch{}}var i=e(n()),a=i.set.bind(i);i.set=e=>{a(e),r(i.get())};function o(){return i.get().isAuthenticated}function s(){return i.get().username}function c(){return i.get().userId}function l(){let e=o()?`
  ...
  `,r.appendChild(i),requestAnimationFrame(()=>{i.style.opacity=`1`}),n>0&&setTimeout(()=>{i.style.opacity=`0`,setTimeout(()=>i.remove(),300)},n);let a=i.querySelector(`.btn-close`);a&&a.addEventListener(`click`,()=>{i.style.opacity=`0`,setTimeout(()=>i.remove(),300)})}var x=null,S=null,C=!1;async function oe(e){if(!C){S=typeof e==`function`?e:null,x=new signalR.HubConnectionBuilder().withUrl(`/hubs/notifications`).withAutomaticReconnect([0,2e3,5e3,1e4,3e4]).configureLogging(signalR.LogLevel.Warning).build(),x.on(`ReceiveNotification`,e=>{S&&S(e)}),x.onreconnecting(async()=>{b(`Reconnecting to server...`,`warning`,0)}),x.onreconnected(async()=>{b(`Reconnected.`,`success`,3e3),S&&S(null)}),x.onclose(async()=>{b(`Connection lost. Real-time updates paused.`,`error`,5e3)});try{await x.start(),C=!0}catch(e){console.warn(`SignalR connection failed, notifications will not be real-time:`,e),b(`Unable to connect to notification service.`,`warning`,5e3),x=null}}}var w=`pmo:notifications:unread-updated`,T=!1;function E(e){let t=document.getElementById(`notification-badge`);if(!t)return;let n=Number.isFinite(e)?e:0;n>0?(t.textContent=String(n),t.classList.remove(`d-none`)):t.classList.add(`d-none`)}async function D(){try{let e=await ee();E(Array.isArray(e)?e.length:0),window.dispatchEvent(new CustomEvent(w,{detail:{notifications:Array.isArray(e)?e:[]}}))}catch{E(0)}}async function se(){await D()}async function ce(){await D(),!T&&(T=!0,oe(()=>{D()}))}function le(){return w}function ue(){let e=window.location.pathname;document.querySelectorAll(`#main-menu a[data-nav]`).forEach(t=>{let n=t.getAttribute(`href`);if(!n||n===`#`)return;let r=n===e||n!==`/`&&e.startsWith(n);t.removeAttribute(`aria-current`),r&&t.setAttribute(`aria-current`,`page`)})}function de(e,t,n){let r=e.target.closest(`a[data-nav]`);if(!r)return;let i=r.getAttribute(`href`);!i||i===`#`||(e.preventDefault(),q(i,t,n))}function fe(e,t){let n=o()?`authenticated.html`:`anonymous.html`;return fetch(`/templates/navigation/${n}`).then(e=>{if(!e.ok)throw Error(`Network response was not ok`);return e.text()}).then(t=>{e.innerHTML=t,ue(),o()&&ce()}).catch(t=>{throw e.innerHTML=`<h1>Error loading template</h1><p>${t.message}</p>`,t})}function pe(e,t){let n=document.getElementById(`main-menu`);!n||n.dataset.navBound||(n.dataset.navBound=`1`,n.addEventListener(`click`,n=>de(n,e,t)))}function O(){return o()?{allowed:!0}:{allowed:!1,redirect:`/login`}}var me=(e,t,n)=>u(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}`),he=(e,t,n)=>f(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/create`,n),ge=()=>u(`/api/moments/assigned-to-me`),k=(e,t,n,r)=>p(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}/stride-assignment`,{strideId:r}),A=(e,t,n,r)=>p(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}/status`,{newStatus:r}),j=(e,t,n,r)=>p(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}/estimate`,{estimate:r}),M=(e,t,n,r)=>p(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}/type`,{newType:r}),N=(e,t,n,r)=>p(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}/owner`,{userId:r}),P=(e,t,n,r)=>p(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}/description`,{description:r}),F=(e,t,n,r)=>f(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}/tasks`,r),I=(e,t,n,r,i)=>p(`/api/projects/${encodeURIComponent(e)}/${encodeURIComponent(t)}/moments/${n}/tasks/${r}/completion`,{isCompleted:i});function L(e){return String(e).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e])}function _e(e){return`
    ...
    `}function ve({icon:e,title:t,description:n,button:r,colspan:i}){return`
        <tr class="inline-table-empty-row">
            <td colspan="${i}" class="text-center py-5">
                <div class="d-flex flex-column align-items-center gap-3">
                    ${e?`<div class="empty-table-icon"><i class="bi ${L(e)}"></i></div>`:``}
                    ${t?`<h5 class="fw-semibold text-secondary mb-1">${L(t)}</h5>`:``}
                    ${n?`<p class="text-muted mb-2">${L(n)}</p>`:``}
                    ${r?`<button${r.id?` id="${L(r.id)}"`:``} class="${L(r.class||`btn btn-outline-primary rounded-pill mt-2`)}" type="button">${r.icon?`<i class="bi ${L(r.icon)} me-1"></i> `:``}${L(r.text)}</button>`:``}
                </div>
            </td>
        </tr>
    `}function R({icon:e,title:t,description:n,button:r}){return`
    ...
    `}function ye(e,t){let n=document.getElementById(`my-tasks-content`),r=document.getElementById(`error-text`);ge().then(r=>{if(!r||r.length===0){n.innerHTML=R({icon:`bi-list-task`,title:`You have no assigned tasks.`,description:`When a moment is assigned to you, it will appear here.`});return}n.innerHTML=`
            ...
            `,n.addEventListener(`change`,async e=>{let t=e.target;if(t.matches(`.moment-type-select`)){let e=t.closest(`tr`),n=e?.dataset.owner,r=e?.dataset.project;if(!n||!r){console.error(`Cannot determine project for moment type update`);return}let i=parseInt(t.dataset.momentId,10),a=t.value,o=t.dataset.currentType||a;try{await M(n,r,i,a),t.dataset.currentType=a}catch(e){t.value=o,console.error(`Failed to update moment type:`,e)}}}),n.querySelectorAll(`a[moment-seq]`).forEach(n=>{n.addEventListener(`click`,r=>{r.ctrlKey||r.metaKey||r.button===1||(r.preventDefault(),q(`/${n.getAttribute(`data-owner`)}/${n.getAttribute(`data-project`)}/moments/${n.getAttribute(`moment-seq`)}`,e,t))})})}).catch(e=>{r.textContent=`Failed to load your tasks.`,console.error(e)})}var z=!1;function B(e){let t=document.getElementById(`notification-badge`);if(!t)return;let n=Number.isFinite(e)?e:0;n>0?(t.textContent=String(n),t.style.display=`inline`):t.style.display=`none`}function be(){let e=document.getElementById(`notification-badge`);if(!e||e.style.display===`none`)return;let t=parseInt(e.textContent||`0`,10);if(!Number.isFinite(t)||t<=0){B(0);return}B(t-1)}function xe(e){if(!e)return;let t=e.classList.contains(`unread`);e.classList.remove(`unread`);let n=e.querySelector(`td[data-actions="1"]`);n&&(n.textContent=`✓ Read`),t&&be()}function V(e,t){if(e){if(!t||t.length===0){e.innerHTML=R({icon:`bi-bell`,title:`No notifications yet.`,description:`You'll see notifications here when there is activity related to you.`});return}e.innerHTML=`
        <div class="mb-3">
    ...
    `,document.getElementById(`mark-all-read`)?.addEventListener(`click`,async()=>{try{await v();let t=window.scrollY;e.querySelectorAll(`tbody tr`).forEach(e=>{e.classList.remove(`unread`);let t=e.querySelector(`td[data-actions="1"]`);t&&(t.textContent=`✓ Read`)}),B(0),window.scrollTo(0,t)}catch(e){alert(`Failed to mark all as read`),console.error(e)}}),e.querySelectorAll(`.mark-read-btn`).forEach(t=>{t.addEventListener(`click`,async()=>{let n=parseInt(t.dataset.id,10);try{await _(n);let t=window.scrollY;xe(e.querySelector(`tr[data-notification-id="${n}"]`)),window.scrollTo(0,t)}catch(e){alert(`Failed to mark notification as read`),console.error(e)}})})}}async function Se(){let e=document.getElementById(`notifications-list`),t=document.getElementById(`error-text`);if(!(!e||!t)){t.textContent=``;try{V(e,await te()),se()}catch{t.textContent=`Failed to load notifications.`}}}function Ce(e){if(!z){z=!0;let e=le();window.addEventListener(e,e=>{let t=document.getElementById(`notifications-list`);if(!t)return;let n=e?.detail?.notifications;V(t,Array.isArray(n)?n:[])})}Se()}function we(e,t,n){e===`/notifications`?X(`notifications/list.html`,n).then(()=>Ce(n)).catch(Q(n,`notifications`)):Z(n)}var Te=()=>u(`/api/permissions/pending`),Ee=e=>p(`/api/permissions/${e}`,{status:`Active`});function De(e){let t=document.getElementById(`invitations-list`),n=document.getElementById(`error-text`);async function r(){try{let e=await Te();if(!e||e.length===0){t.innerHTML=R({icon:`bi-envelope`,title:`No pending invitations.`,description:`When someone invites you to a project, it will appear here.`});return}t.innerHTML=`
                <table class="table table-sm table-striped table-hover align-middle">
                    <thead>
                        <tr>
                            <th>Project</th>
            ...
            `,document.querySelectorAll(`.accept-btn`).forEach(e=>{e.addEventListener(`click`,async()=>{let n=parseInt(e.dataset.permissionId,10);try{await Ee(n);let r=window.scrollY;e.closest(`tr`)?.remove(),t.querySelectorAll(`tbody tr`).length===0&&(t.innerHTML=R({icon:`bi-envelope`,title:`No pending invitations.`,description:`When someone invites you to a project, it will appear here.`})),window.scrollTo(0,r)}catch(e){alert(`Failed to accept invitation`),console.error(e)}})})}catch{n.textContent=`Failed to load invitations.`}}r()}function Oe(e,t){e===`/invitations`?X(`invitations/list.html`,t).then(()=>De(t)).catch(Q(t,`invitations`)):Z(t)}function H(e,t,n){return`
    <section id="${e}" class="kb-section">
      <h1>${t}</h1>
      ${n}
    </section>
  `}function ke(){let e=document.getElementById(`kb-content`);e.innerHTML=[H(`overview`,`The Promise Stack Overview`,`

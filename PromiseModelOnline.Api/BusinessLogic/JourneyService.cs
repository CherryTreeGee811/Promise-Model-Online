...
public class JourneyService(IJourneyRepository journeyRepository, IHierarchyStatusService hierarchyStatusService) : GenericService<Journey>(journeyRepository), IJourneyService
{
    ...
    public async Task<IEnumerable<Journey>> GetJourneysByEpicAsync(int epicId, CancellationToken cancellationToken = default)
        => await _journeyRepository.GetJourneysByEpicAsync(epicId, cancellationToken);
    ...
    /// <param name="cancellationToken">Cancellation token.</param>
    public override async Task AddAsync(Journey entity, CancellationToken cancellationToken = default)
    {
        await base.AddAsync(entity, cancellationToken);
        await _hierarchyStatusService.RecalculateFromJourneyAsync(entity.Id, cancellationToken);
    }
    ...
    public override async Task<bool> DeleteByIdAsync(object id, CancellationToken cancellationToken = default)
    {
        ...
        if (deleted)
        {
            await _hierarchyStatusService.RecalculateFromEpicAsync(journey.EpicId, cancellationToken);
        }

...
public class ProjectMomentTasksIntegrationTests : ApiIntegrationTestBase
{
    ...
    private async Task SeedHierarchyAsync()
    {
        ...
        await db.SaveChangesAsync();
        _momentSeq = moment.SequenceNumber;
        _momentId = moment.Id;
    }
    ...
    public async Task CreateTask_AsOwner_ReturnsOk()
    {
        ...
        // Act
        var response = await PostAsync($"/api/projects/{Owner}/{Project}/moments/{_momentSeq}/tasks",
    ...
    public async Task CreateTask_NullBody_Returns400()
    {
        ...
        // Act
        var response = await Client.PostAsync($"/api/projects/{Owner}/{Project}/moments/{_momentSeq}/tasks", json);
        // Assert
        Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.BadRequest));
    }

    [Test]
    [Description("REQ_FUN_011 sad path: Non-existent moment returns 404")]
    public async Task CreateTask_NonExistentMoment_Returns404()
    {
        // Arrange
        SetAuthHeader(OwnerToken);
        // Act
        var response = await PostAsync($"/api/projects/{Owner}/{Project}/moments/999/tasks",
            new { name = "Bad moment", isCompleted = false });
        // Assert
        // Project-scoped controller finds moment by seq; non-existent seq => NotFound
    ...
    public async Task UpdateCompletion_AsOwner_ReturnsOk()
    {
        ...
        SetAuthHeader(OwnerToken);
        // Act
        var response = await PatchAsync($"/api/projects/{Owner}/{Project}/moments/{_momentSeq}/tasks/{seeded.Id}/completion",
    ...
    public async Task UpdateCompletion_Unauthenticated_Returns401()
    {
        ...
        // (no auth header set)
        // Act
        var response = await PatchAsync($"/api/projects/{Owner}/{Project}/moments/{_momentSeq}/tasks/1/completion",
            new { isCompleted = true });
        // Assert
        Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.Unauthorized));
    }
    ...
    public async Task UpdateCompletion_NonExistentTask_Returns404()
    {
        ...
        SetAuthHeader(OwnerToken);
        // Act
        var response = await PatchAsync($"/api/projects/{Owner}/{Project}/moments/{_momentSeq}/tasks/999/completion",
            new { isCompleted = true });
        // Assert
        Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.NotFound));
    }
